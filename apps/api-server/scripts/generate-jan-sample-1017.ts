/**
 * Generate JAN (Jabatan Akauntan Negara) sample data - 1017 records in encrypted .gpg format,
 * plus 1 bank statement row that matches the total of all 1017 JAN records.
 *
 * Format: pipe-delimited | NOIC | NAMA | TARIKH | RUJUKAN | AMAUN |
 *
 * Usage: npx tsx scripts/generate-jan-sample-1017.ts [--seed-db]
 *
 * Output:
 *   - uploads/samples/jan-sample-1017.txt.gpg (PGP encrypted)
 *   - uploads/samples/jan-sample-1017-keys.txt (password for decryption)
 *   - uploads/samples/bank-statement-jan-1017.csv (1 row, amount = sum of 1017 JAN records)
 *
...
async function main() {
  ...
  const encPath = path.join(outDir, "jan-sample-1017.txt.gpg");
  const keysPath = path.join(outDir, "jan-sample-1017-keys.txt");
  const bankCsvPath = path.join(outDir, "bank-statement-jan-1017.csv");
...
`,
  ...
  // Bank statement CSV: 1 row matching total of 1017 JAN records
  const bankHeader = "paymentReference,approvalCode,chequeNo,valueDate,amount,description,counterparty";
  const bankRef = `JAN-BATCH-${year}${month}-1017`;
  const bankRow = [
    bankRef,
    "",
    "",
  ...
  console.log(`  - 1 row, amount RM ${totalAmount.toFixed(2)} (matches sum of 1017 JAN records)`);
    ...
    try {
      ...
      await prisma.bankStatementTransaction.create({
        data: {
          ...
          amount: totalAmount,
          description: "Zakat JAN batch 1017 records",
  ...
  console.log(`\nUpload jan-sample-1017.txt.gpg via File Upload (source=JAN, fileType=ENCRYPTED_TXT)`);
}
