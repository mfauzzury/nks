...
export default function IndividualPaySuccessPage() {
  ...
  const [phase, setPhase] = useState<"processing" | "receipt">("processing");
  const [processingText, setProcessingText] = useState("Menghantar maklumat bayaran...");
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const response = await getGuestReceipt(Number(receiptId));
      ...
      <div className="mx-auto w-full max-w-6xl space-y-6 px-4 md:px-6">
        ...
        {phase === "processing" && (
          <div className="flex flex-col items-center justify-center rounded-2xl bg-white p-16 shadow-2xl">
            ...
            <p className="text-xl font-bold text-slate-800">Memproses Bayaran</p>
            <p className="mt-2 text-base text-slate-500">{processingText}</p>
          </div>
        )}
        ...
        {phase === "receipt" && !receipt && !error && (
          <div className="rounded-2xl bg-white p-8 text-center text-sm text-slate-500 shadow-2xl">
            Receipt ID tidak ditemui.
          </div>
        )}
      </div>
    </div>
