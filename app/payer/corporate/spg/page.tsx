          ...
          <div key={s.num} className="z-10 flex flex-col items-center gap-1.5">
            ...
            <span
              className={`text-xs font-medium ${
                done || active ? "text-[#1f4ed8]" : "text-slate-400"
              }`}
            >
              {s.label}
            </span>
...
export default function CorporateSpgPage() {
  ...
  return (
    <PortalAuthGuard expected="corporate">
      <div className="space-y-6">
        {/* Page header */}
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Urus SPG Bulanan</h1>
                  ...
                  {validatedRows.map((row, idx) => (
                    <tr key={row.id} className="group">
                      ...
                      <td className="px-2 py-2">
                        <Input
                          className="h-8 text-sm"
                          value={row.employeeIdentityNo}
                          onChange={(e) => updateRow(row.id, { employeeIdentityNo: e.target.value })}
                          placeholder="No. IC"
                        />
                      </td>
                      <td className="px-2 py-2">
                      ...
                      <td className="px-2 py-2 text-right">
                        <button
                          ...
                          className="inline-flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 opacity-0 transition hover:bg-red-50 hover:text-red-600 group-hover:opacity-100"
                          title="Buang baris"
                        >
        ...
        {step === 4 && (
          <div className="rounded-2xl border border-slate-200 bg-white">
            ...
            <div className="space-y-5 px-6 py-5">
              {/* Payment channels as selectable cards */}
              <div>
            ...
            <div className="flex items-center justify-between border-t border-slate-100 px-6 py-4">
              ...
              <Button
                ...
                className="gap-2"
                onClick={() => setStep(5)}
                disabled={paymentChannel === "CHEQUE" && !supportingSlip}
              >
                Seterusnya
                <ArrowRight className="h-4 w-4" />
              </Button>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white">
          <div className="border-b border-slate-100 px-6 py-4">
            <h2 className="text-base font-semibold text-slate-900">Sejarah Batch SPG</h2>
            <p className="text-sm text-slate-500">Senarai batch potongan gaji yang telah dihantar.</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {batches.map((row) => {
                  const statusColor =
                    row.status === "paid_success"
                      ? "bg-emerald-100 text-emerald-700"
                      : row.status === "paid_failed" || row.status === "cancelled"
                        ? "bg-red-100 text-red-700"
                        : "bg-slate-100 text-slate-700";
                  const statusLabel =
                    row.status === "draft_preview" ? "Draf"
                    : row.status === "awaiting_online_payment" ? "Menunggu Bayaran Online"
                    : row.status === "pending_payment" ? "Menunggu Bayaran"
                    : row.status === "paid_success" ? "Berjaya"
                    : row.status === "paid_failed" ? "Gagal"
                    : row.status === "cancelled" ? "Dibatalkan"
                    : row.status;
                  return (
                    <tr key={row.id} className="hover:bg-slate-50">
                      <td className="px-4 py-3 font-medium text-slate-800">{row.referenceNo}</td>
                      ...
                      <td className="px-4 py-3 text-right font-semibold text-slate-900">{moneyFormat(Number(row.totalAmount))}</td>
                ...
                {batches.length === 0 && (
                  <tr>
                    <td colSpan={6} className="px-4 py-8 text-center text-sm text-slate-400">Tiada batch SPG lagi.</td>
                  </tr>
                )}
              </tbody>
            </table>
