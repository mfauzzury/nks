...
import { Download, Eye, FileText, PieChart as PieChartIcon, ReceiptText } from "lucide-react";
...
const PIE_COLORS = ["#1f4ed8", "#3b82f6", "#60a5fa", "#93c5fd", "#2563eb", "#1d4ed8", "#6366f1", "#818cf8", "#a5b4fc", "#4f46e5"];
...
export default function CorporateRecordsPage() {
  ...
  const companyName = session?.companyName || session?.displayName || "";
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-foreground">Rekod Sumbangan Korporat</h1>
            <p className="text-sm text-muted-foreground">Sila log masuk untuk melihat rekod syarikat.</p>
          </div>
        </div>
        <section className="rounded-2xl border border-slate-200 bg-white p-6">
          <p className="text-sm text-slate-600">Akses rekod memerlukan sesi portal korporat yang sah.</p>
          <div className="mt-4">
            <Link href="/portal/login" className="rounded-lg bg-[#1f4ed8] px-4 py-2 text-sm font-semibold text-white">
              Log Masuk
            </Link>
          </div>
        </section>
      ...
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <select
            ...
            className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 focus:border-[#1f4ed8] focus:outline-none focus:ring-1 focus:ring-[#1f4ed8]"
          >
        ...
        <section className="grid gap-4 md:grid-cols-3">
          <article className="rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <p className="mt-2 text-2xl font-semibold text-slate-900">{loading ? "..." : totalTransactions}</p>
          </article>
          <article className="rounded-2xl border border-slate-200 bg-white p-5">
            <p className="text-sm text-slate-500">Jumlah Sumbangan</p>
            <p className="mt-2 text-2xl font-semibold text-slate-900">{loading ? "..." : formatCurrency(filteredTotalAmount)}</p>
          </article>
          <article className="rounded-2xl border border-slate-200 bg-white p-5">
            <p className="text-sm text-slate-500">Resit Terkini</p>
            <p className="mt-2 text-base font-semibold text-slate-900">{loading ? "..." : latestReceipt}</p>
            <p className="mt-1 text-xs text-slate-500">{filteredLatestPaidAt ? formatDate(filteredLatestPaidAt) : "-"}</p>
          </article>
        </section>
        ...
        {!loading && chartData.length > 0 && (
          <section className="rounded-2xl border border-slate-200 bg-white p-6">
            <div className="mb-4 flex items-center gap-2">
              <PieChartIcon className="h-5 w-5 text-[#1f4ed8]" />
              <h2 className="text-base font-semibold text-slate-900">Pecahan Jenis Sumbangan</h2>
            </div>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <ReceiptText className="h-5 w-5 text-[#1f4ed8]" />
                      ...
                      <tr key={row.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-3 py-2 text-right">
                          <button
                            ...
                            className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 transition hover:bg-slate-100 hover:text-[#1f4ed8]"
                          >
              ...
              <>
                <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                  <div className="flex items-center gap-2.5">
                    <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-blue-100 text-[#1f4ed8]">
                ...
                <div className="space-y-4 px-6 py-5">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div className="rounded-lg bg-slate-50 p-3">
                      <p className="text-xs text-slate-400">Nama Syarikat</p>
                      <p className="mt-0.5 font-medium text-slate-700">{selectedTx.guestName}</p>
                    </div>
                    <div className="rounded-lg bg-slate-50 p-3">
                      <p className="text-xs text-slate-400">No. SSM</p>
                ...
                <div className="flex items-center justify-end gap-2 border-t border-slate-100 px-6 py-4">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-lg bg-[#1f4ed8] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#1a44ba]"
                  >
