...
export function ProcessSteps() {
  return (
    <section className="rounded-3xl border border-slate-200 bg-[#1a0538] p-7 text-white md:p-10">
      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#c9a5ff]">Aliran Pembayaran</p>
      ...
      <div className="mt-7 grid gap-4 md:grid-cols-3">
        {steps.map((step, idx) => (
          <div key={step.title} className="rounded-2xl border border-white/15 bg-white/5 p-4">
            <p className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-[#7E30E1] text-xs font-semibold">
              {idx + 1}
            </p>
